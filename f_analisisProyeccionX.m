...
medUmbral = med*.2 %umbral calculado, equivale al 2% de la altura promedio del histograma de la imagen.
...
[altura x] = size (iLetrasSeg); %en el original se usaba iLetrasSeg, ahora se utiliza placa_Original2, que tiene las mismas dimensiones por ello no se cambia la variable
i=1;
H = x;
while(i<H)
    if(histX(i)>0)
        ...
        if(i>1 && (histX(i-1)==0) && (histX(i+1)>0))
           ...
           % iLetrasNumeros = imcrop(placa_Original,[xi-5 0 (xf-xi)+5 altura ]);
             ...
             NormalizarCaracter2(iLetrasNumeros);
