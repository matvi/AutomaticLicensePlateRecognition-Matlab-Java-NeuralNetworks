...
public class MainEntry extends JFrame implements Runnable {
   ...
   MainEntry()
   {
     ...
     entry = new Entry();//el lugar donde escribes
     entry.reshape(168,25,200,128);
     getContentPane().add(entry);

//     sample = new Sample(DOWNSAMPLE_WIDTH,DOWNSAMPLE_HEIGHT); //es el area de cuadricula
//     sample.reshape(307,210,65,70);
//     entry.setSample(sample);
//     getContentPane().add(sample);
     ALPR alpr = new ALPR();
     alpr.reshape(307, 210, 150, 150);
     getContentPane().add(alpr);
